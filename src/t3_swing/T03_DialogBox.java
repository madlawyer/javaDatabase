	...
	// -----------------------위쪽은 사용자 인터페이스구현(UI), 아래쪽은 method(동작) 처리 ---------------------		
		...
		btn1.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				//JOptionPane.showMessageDialog(null, lblMessage.getText()); //null을 쓰면 가운데에 뜸
				JOptionPane.showMessageDialog(null, lblMessage.getText(), "경고메세지", JOptionPane.WARNING_MESSAGE); // 아이콘 바꾸기
			}
		});
		...
		btn2.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String name = JOptionPane.showInputDialog(null, "성명을 입력하세요", "입력창", JOptionPane.DEFAULT_OPTION); // 입력창은 제목 네번째는 기호
		...
		btn4.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int ans = JOptionPane.showConfirmDialog(null, "작업을 계속 하시겠습니가?", "작업 종료창", JOptionPane.YES_NO_OPTION); //cofrim : 예 아니오
				if(ans == 0) {
					JOptionPane.showMessageDialog(null, "작업을 계속 진행합니다.");
				}
				else {
					JOptionPane.showMessageDialog(null, "마칩니다.");
					System.exit(0);
				}
			}
		});
		...
		btn5.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int ans = JOptionPane.showConfirmDialog(null, "작업을 계속 하시겠습니가?", "작업 종료창", JOptionPane.YES_NO_CANCEL_OPTION); //cofrim : 예 아니오
				if(ans == 0) {
					JOptionPane.showMessageDialog(null, "작업을 계속 진행합니다.");
				}
				else if(ans == 1) {
