...
-- 필드 추가 (add column) ddl(creat drop alter)
...
insert into test1 values ('순식나',default,default,'', default,'제주');
 ...
 insert into test1 values ('씨톰',55,'2023-01-11','디자이너', '남자','서울');
 insert into test1 values ('디톰',66,default,'장군', default,'울산');
 insert into test1 values ('이톰','43',default,'', '남자','경기도');
 insert into test1 values ('이톰','43',default,'', '남자','울산');
 insert into test1 values ('홍톰','43',default,'', '남자','울산');
 ...
 select * from test1 where birthday < '2000-01-01';
 select * from test1 where birthday >= '2025-01-01' and birthday <= '2025-12-31';
 select * from test1 where job = '회사원' and gender = '남자';
 select * from test1 where name like '홍%';
 select * from test1 where name like '%나%'and address = '청주'; /* %나와도 되고 안나와도됨 */
